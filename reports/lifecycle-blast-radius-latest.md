...
Generated: 2026-05-19T12:35:44.359Z
Status: pass
...
| Check | Status | Detail |
|---|---:|---|
| paid-server-registers-disabled-without-output-secret-leak | pass | exit=0 24ms |
| server-enable-is-explicit-state-transition | pass | exit=0 24ms |
| server-disable-is-explicit-state-transition | pass | exit=0 25ms |
| server-remove-dry-run-does-not-delete | pass | exit=0 28ms |
| server-remove-deletes-only-target-entry | pass | exit=0 23ms |
| server-can-be-readded-after-remove | pass | exit=0 21ms |
| normalized-duplicate-without-force-blocked | pass | exit=1 server 'Paid Billing' already exists in C:\Users\rmatv\AppData\Local\Temp\mcpace-lifecycle-blast-cBAFK3\mcp_settings.d\paid-billing.json; rerun with --force to replace it |
...
| supply-chain-package-launchers-are-documented-risk | pass | 3 automatic install launchers are present; docs must explicitly call out package-manager risk. |
