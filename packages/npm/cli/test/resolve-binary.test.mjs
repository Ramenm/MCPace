...
function writeSourceWorkspaceMarker(root) {
	...
	fs.writeFileSync(
		...
		'{"name":"mcpace-workspace","version":"0.6.9"}\n',
		"utf8",
	);
}
...
test("resolveBinary prefers MCPACE_BINARY_PATH", async (t) => {
	...
	const bin = createExecutableFixture(
		path.join(tmp, process.platform === "win32" ? "mcpace.exe" : "mcpace"),
	);
...
test("resolveBinary prefers a vendored binary from the workspace repo", () => {
	...
	try {
		assert.equal(
			resolveBinary({
				repoRoot: tmp,
				packageRoot: path.join(tmp, "unused-package-root"),
...
test("resolveBinary identifies malformed launcher metadata", () => {
	const target = detectTarget();
	if (!target) return;

	const tmp = fs.mkdtempSync(
		path.join(os.tmpdir(), "mcpace-malformed-metadata-"),
	);
	const packageRoot = path.join(tmp, "node_modules", "@mcpace", "cli");
	try {
		fs.mkdirSync(packageRoot, { recursive: true });
		fs.writeFileSync(path.join(packageRoot, "package.json"), "{ malformed");
		assert.throws(
			() =>
				resolveBinary({
					repoRoot: path.join(tmp, "repo-root"),
			...
			/main package\.json is not valid JSON:/,
		);
	} finally {
		fs.rmSync(tmp, { recursive: true, force: true });
	}
...
function writeOptionalBinaryPackage(packageRoot, target, overrides = {}) {
	const pkgName =
		target.packageName ?? target.npmPackage ?? `@mcpace/cli-${target.key}`;
	...
	const defaultMcpace =
		target.platform === "win32"
			? {
					...
					sidecarBinaries: ["mcpace-agent-launcher.exe"],
				}
			: { target: target.key };
	const packageJson = {
		name: pkgName,
	...
	if (target.platform === "win32" && overrides.createSidecar !== false) {
		createExecutableFixture(
			path.join(pkgRoot, "bin", "mcpace-agent-launcher.exe"),
			"launcher fixture\n",
		);
	}
...
function windowsTarget() {
	return SUPPORTED_TARGETS.find((target) => target.key === "win32-x64-msvc");
}
...
test("resolveBinary requires matching release SHA when the launcher declares one", () => {
	const target = detectTarget();
	if (!target) return;

	const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "mcpace-optional-sha-"));
	const packageRoot = path.join(tmp, "node_modules", "@mcpace", "cli");
	const launcherSha = "a".repeat(40);
	try {
		writeMainCliPackage(packageRoot, "0.7.3", launcherSha);
		const { bin } = writeOptionalBinaryPackage(packageRoot, target, {
			packageJson: {
				mcpace: {
					target: target.key,
					releaseSha: launcherSha,
					...(target.platform === "win32"
						? {
								binaryName: binaryNameForTarget(target),
								sidecarBinaries: ["mcpace-agent-launcher.exe"],
...
test("resolveBinary rejects a native package from a different release SHA", () => {
	const target = detectTarget();
	if (!target) return;

	const tmp = fs.mkdtempSync(
		path.join(os.tmpdir(), "mcpace-optional-sha-mismatch-"),
	);
	const packageRoot = path.join(tmp, "node_modules", "@mcpace", "cli");
	try {
		writeMainCliPackage(packageRoot, "0.7.3", "a".repeat(40));
		writeOptionalBinaryPackage(packageRoot, target, {
			packageJson: {
				mcpace: {
					target: target.key,
...
test("resolveBinary rejects installed optional native packages with version drift", () => {
	...
	const tmp = fs.mkdtempSync(
		path.join(os.tmpdir(), "mcpace-optional-version-"),
	);
	const packageRoot = path.join(tmp, "node_modules", "@mcpace", "cli");
	try {
		writeMainCliPackage(packageRoot);
...
test("resolveBinary rejects installed optional native packages with missing or wrong target metadata", () => {
	...
	try {
		writeMainCliPackage(packageRoot);
		writeOptionalBinaryPackage(packageRoot, target, {
			packageJson: { mcpace: { target: "wrong-target" } },
		});
		assert.throws(
			() =>
				resolveBinary({
...
test("resolveBinary rejects symlinked binaries inside installed optional native packages", (t) => {
	...
	const tmp = fs.mkdtempSync(
		path.join(os.tmpdir(), "mcpace-optional-symlink-"),
	);
	const packageRoot = path.join(tmp, "node_modules", "@mcpace", "cli");
	try {
		writeMainCliPackage(packageRoot);
...
test("resolveBinary rejects symlinked installed optional package.json metadata", (t) => {
	...
	const tmp = fs.mkdtempSync(
		path.join(os.tmpdir(), "mcpace-optional-pkgjson-symlink-"),
	);
	const packageRoot = path.join(tmp, "node_modules", "@mcpace", "cli");
	try {
		writeMainCliPackage(packageRoot);
...
test("resolveBinary rejects a native package linked outside node_modules", (t) => {
	const target = detectTarget();
	if (!target) return;
	const tmp = fs.mkdtempSync(
		path.join(os.tmpdir(), "mcpace-package-parent-link-"),
	);
	const packageRoot = path.join(tmp, "node_modules", "@mcpace", "cli");
	try {
		writeMainCliPackage(packageRoot);
		const { pkgRoot } = writeOptionalBinaryPackage(packageRoot, target);
		const outside = path.join(tmp, "outside-native-package");
		fs.renameSync(pkgRoot, outside);
		try {
			fs.symlinkSync(
				outside,
				pkgRoot,
			...
			t.skip(`directory links are unavailable: ${error?.message || error}`);
			return;
		}
		assert.throws(
			() =>
			...
			/escapes approved node_modules roots/,
		);
	} finally {
		fs.rmSync(tmp, { recursive: true, force: true });
	}
...
test("resolveBinary permits pnpm-style links that remain under node_modules", (t) => {
	const target = detectTarget();
	if (!target) return;
	const tmp = fs.mkdtempSync(
		path.join(os.tmpdir(), "mcpace-package-pnpm-link-"),
	);
	...
	try {
		writeMainCliPackage(packageRoot);
		const { pkgRoot, bin } = writeOptionalBinaryPackage(packageRoot, target);
		...
		fs.mkdirSync(path.dirname(actualRoot), { recursive: true });
		fs.renameSync(pkgRoot, actualRoot);
		try {
			fs.symlinkSync(
				actualRoot,
				pkgRoot,
		...
		assert.equal(
			resolved,
			fs.realpathSync(path.join(actualRoot, path.relative(pkgRoot, bin))),
		);
	} finally {
		fs.rmSync(tmp, { recursive: true, force: true });
	}
...
test("resolveBinary rejects Windows optional native packages without the hidden autostart launcher sidecar", () => {
	const target = windowsTarget();
	assert.ok(target, "win32-x64-msvc target must be present");

	const tmp = fs.mkdtempSync(
		path.join(os.tmpdir(), "mcpace-optional-win-sidecar-missing-"),
	);
	const packageRoot = path.join(tmp, "node_modules", "@mcpace", "cli");
	try {
		writeMainCliPackage(packageRoot);
		writeOptionalBinaryPackage(packageRoot, target, { createSidecar: false });
		assert.throws(
			() =>
				resolveBinary({
					repoRoot: path.join(tmp, "repo-root"),
			...
			/Windows autostart sidecar does not exist/,
		);
	} finally {
		fs.rmSync(tmp, { recursive: true, force: true });
	}
...
test("resolveBinary rejects Windows optional native packages without sidecar metadata", () => {
	const target = windowsTarget();
	assert.ok(target, "win32-x64-msvc target must be present");

	const tmp = fs.mkdtempSync(
		path.join(os.tmpdir(), "mcpace-optional-win-sidecar-metadata-"),
	);
	const packageRoot = path.join(tmp, "node_modules", "@mcpace", "cli");
	try {
		writeMainCliPackage(packageRoot);
		writeOptionalBinaryPackage(packageRoot, target, {
			packageJson: {
				mcpace: { target: target.key, binaryName: binaryNameForTarget(target) },
			},
		});
		assert.throws(
			() =>
...
test("resolveBinary accepts Windows optional native package only when the hidden launcher sidecar is present and declared", () => {
	const target = windowsTarget();
	assert.ok(target, "win32-x64-msvc target must be present");

	const tmp = fs.mkdtempSync(
		path.join(os.tmpdir(), "mcpace-optional-win-sidecar-ok-"),
	);
	const packageRoot = path.join(tmp, "node_modules", "@mcpace", "cli");
	try {
		writeMainCliPackage(packageRoot);
