...
| P0 | R3 | partial | Enforce scheduler leases for `single-session` / `shared-exclusive` / browser-profile / project-local servers | 8–13 | current planner warnings | file-backed acquire/renew/release/list exists with TTL, conflict checks, request-time upstream forwarding gates, conservative settings-only leases, heartbeat renewal, lost-heartbeat cancellation, and tests; remaining: takeover semantics for future long-lived pooled sessions | stale ownership in future pooled sessions, deadlocks, restart races | next runtime pass |
| P0 | R4 | partial | Add cancel/restart guards so dead leases cannot accept stale results | 8–13 | R3, ingress work | request-time wrapper calls now ignore stale JSON-RPC ids and cancel before accepting a result after lease heartbeat loss; remaining: transport-level cancel propagation, restart/session registry, and adversarial lab fixtures for pooled sessions | subtle race conditions and transport mismatch | 1–2 weeks |
...
| P1 | A2 | partial | Prove config-merge safety and owned-block patching against real-looking client configs | 5–8 | A1 | fixtures include mixed user-managed config; patcher preserves non-MCPace blocks; rollback path exists for current install backups; remaining: broaden real client fixtures | destructive config writes; incomplete client samples | 1–2 weeks |
...
| P1 | B1 | partial | Re-run build proof across supported OS lanes | 8–13 | Rust toolchain, CI hosts | current Windows/container host passes `cargo fmt --check`, `cargo clippy --all-targets --locked -- -D warnings`, `cargo test --all-targets --locked`, and `cargo build --release`; remaining: repeated Linux/macOS/Windows release-lane proof | host drift; CI lane availability | 1–2 weeks once remaining hosts exist |
...
1. The next best runtime step is now **durable ownership beyond the request lease gate**: add pooled upstream instance ownership, takeover semantics, and cross-request stale-result/cancel guards around the already lease-gated/lost-heartbeat-guarded wrapper calls.
