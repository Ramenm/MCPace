...
| Streamable HTTP remote with possible session state | `P2_session_safe`, `remote-http-session-pool` | Streamable HTTP may issue transport session ids, so unknown remotes are session-aware until an explicit stateless policy exists. |
| Explicit stateless Streamable HTTP remote | `P4_stateless_remote_candidate`, `remote-http-shared-pool`, higher in-flight limit | Only `stateless: true` plus `stateBinding: none` may use the faster stateless lane. |
...
| Memory/context-store server | `P2_session_safe`, `singleton` | Stateful scratchpads and memory stores need a state-profile/session key before parallel scheduling. |
| Local database server | `P3_project_safe`, `project-pool`, project/db/credential locks | Database state is project/resource-bound even when a credential is also present. |

## Non-goals for this source-level matrix

This matrix does not execute third-party MCP packages, make paid API calls, perform browser actions against production sites, or prove Rust runtime compilation. Those are live-host lanes. It validates automatic source-evidence classification and does not rely on a packaged upstream-server catalog.
