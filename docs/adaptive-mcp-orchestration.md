...
This document is the implementation-facing design for MCPace adaptive parallelism. It replaces the old idea of one static `concurrencyPolicy` or packaged catalog as the source of truth with an evidence-backed server profile and worker plan.

## Design goal

MCPace should maximize throughput through isolation, not blind trust. Unknown servers start conservative. Servers get more concurrency only after source evidence, explicit operator policy metadata, safe probes, and runtime evidence support it. Any security, cost, credential, state-leak, or crash signal can only downgrade concurrency until an operator or later evidence raises confidence again. There is no packaged upstream-server catalog; the implementation report is generated by `scripts/adaptive-parallelism-audit.mjs`.
...
| `P4_stateless_remote_candidate` | Explicitly stateless remote API | shared remote pool with rate and credential limits; not used for remote servers that may carry session state |
...
| `remote-http-shared-pool` | Explicit stateless remote HTTP with provider budget boundaries |
...
Writes and destructive tools require a lock even if the server profile looks parallel-safe. Unknown tools are never upgraded purely from names or descriptions. Remote Streamable HTTP servers also require a transport-session or credential/provider lock unless their policy explicitly says `stateless: true` with `stateBinding: none`.
