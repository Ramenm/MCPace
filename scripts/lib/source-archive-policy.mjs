...
export function sourceArchivePolicyIssue(entryPath, options = {}) {
  ...
  const candidates = options.allowSingleRoot === false
    ? [normalized]
    : [normalized, stripSingleRoot(normalized)];

  const lowerCandidates = candidates
    .map((candidate) => normalizeArchivePath(candidate).toLocaleLowerCase('en-US'))
    .filter(Boolean);

  if (lowerCandidates.some((candidate) => ALLOWED_SOURCE_ARCHIVE_DATABASE_PATHS.includes(candidate))) {
    return null;
  }
  ...
  for (const relativePath of lowerCandidates) {
    ...
    for (const prefix of FORBIDDEN_SOURCE_ARCHIVE_PREFIXES) {
      const normalizedPrefix = normalizeArchivePath(prefix).toLocaleLowerCase('en-US');
...
export function sourceArchivePolicyViolations(entries, options = {}) {
  ...
  for (const entry of entries) {
    ...
    if (issue) violations.push({ path: normalizeArchivePath(entry), reason: issue });
  }
...
export function assertSourceArchivePolicy(entries, options = {}) {
  ...
  if (violations.length > 0) {
    throw new Error(`source archive contains generated/runtime artifacts: ${JSON.stringify(violations, null, 2)}`);
  }
  return true;
}
