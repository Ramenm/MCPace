...
function validateWorkflow(relativePath, environment) {
  ...
  const checks = [
    ...
    [/npm exec --yes --package=npm@11\.13\.0 -- npm --version/, 'publish workflow must verify the exact npm version without global installation'],
    [/MCPACE_NPM_EXEC_PACKAGE:\s*npm@11\.13\.0/, 'publish workflow must run publish commands through the exact npm package'],
