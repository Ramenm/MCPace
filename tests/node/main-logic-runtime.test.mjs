...
test("main read-modify-write flows take locks before reading current state", () => {
	...
	const clientLock = indexOfOrThrow(
		clientActions,
		'acquire_exclusive_file_lock(\n                &self.config_path,\n                "client config install",',
	);
	const clientRead = indexOfOrThrow(
		clientActions,
		"fs::read_to_string(&self.config_path)",
	);
	const clientWrite = indexOfOrThrow(
		clientActions,
		"runtimepaths::write_text_atomic(&self.config_path, &update.contents)",
	);
	assert.ok(
		clientLock < clientRead,
		"client config install lock must be acquired before reading config",
	);
	assert.ok(
		clientRead < clientWrite,
		"client config install must write atomically after computing update",
	);

	const policy = read("src/server/policy.rs");
	const policyLock = indexOfOrThrow(
		policy,
		'acquire_exclusive_file_lock(&config_path, "server policy update")',
	);
	const policyRead = indexOfOrThrow(policy, "fs::read_to_string(&config_path)");
	const policyWrite = indexOfOrThrow(
		policy,
		"runtimepaths::write_text_atomic(\n            &config_path,",
	);
	assert.ok(
		policyLock < policyRead,
		"server policy lock must be acquired before reading mcpace.config.json",
	);
	assert.ok(
		policyRead < policyWrite,
		"server policy update must write atomically after parsing current config",
	);

	const projects = read("src/projects.rs");
	const upsertProject = projects.slice(
		indexOfOrThrow(
			projects,
			"fn upsert_project(path: &Path, summary: &ProjectSummary)",
		),
	);
	...
	const projectRead = indexOfOrThrow(
		upsertProject,
		"json_helpers::read_json_file(path)?",
	);
	const projectWrite = indexOfOrThrow(
		upsertProject,
		"runtimepaths::write_text_atomic(path, &root.to_pretty_string())",
	);
	assert.ok(
		projectLock < projectRead,
		"project registry lock must be acquired before reading registry",
	);
	assert.ok(
		projectRead < projectWrite,
		"project registry update must write atomically after merge",
	);
});

test("setup home import participates in the same MCP settings namespace contract", () => {
	const setup = read("src/setup.rs");
	const namespaceLock = indexOfOrThrow(
		setup,
		"mcp_sources::acquire_mcp_settings_namespace_lock(root_path)",
	);
	...
	const collect = indexOfOrThrow(
		setup,
		"let sources = collect_existing_home_mcp_sources(root_path, warnings);",
	);
	const write = indexOfOrThrow(
		setup,
		"runtimepaths::write_private_text_atomic(&target_path, &serialized)",
	);
	assert.ok(
		namespaceLock < targetLock,
		"home import must acquire namespace lock before target-file lock",
	);
	assert.ok(
		targetLock < collect,
		"home import should collect and decide while the MCP namespace is locked",
	);
	assert.ok(
		collect < write,
		"home import writes the computed import file atomically",
	);
});

test("serve/dashboard boundary is loopback-only and request-gated", () => {
...
test("release lane still publishes native packages before the launcher package", () => {
	...
	const nativePublish = indexOfOrThrow(
		workflow,
		"Publish or resume native npm package set",
	);
	const launcherPublish = indexOfOrThrow(
		workflow,
		"Publish main npm launcher last",
	);
	assert.ok(
		nativeJob < publishJob,
		"workflow must build native packages before publish job",
	);
	assert.ok(
		nativePublish < launcherPublish,
		"native packages must publish before launcher",
	);
