...
test('declared servers without policy still inherit generic source policy inference', () => {
  ...
  assert.match(normalizeBody, /policy_token\(policy,\s*"scopeClass",\s*inferred_policy\.scope_class\)/);
  assert.match(normalizeBody, /policy_token\(\s*policy,\s*"concurrencyPolicy",\s*inferred_policy\.concurrency_policy/s);
  assert.match(normalizeBody, /policy_usize\(\s*policy,\s*"parallelismLimit",\s*inferred_policy\.parallelism_limit/s);
  assert.doesNotMatch(normalizeBody, /policy_token\(policy,\s*"scopeClass",\s*""\)/);
  assert.doesNotMatch(normalizeBody, /policy_token\(policy,\s*"concurrencyPolicy",\s*""\)/);
});
