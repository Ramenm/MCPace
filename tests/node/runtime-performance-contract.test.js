...
test('runtime benchmark helper is wired into package scripts and documents operator usage', () => {
  ...
  const perfVerification = read('docs/performance-verification.md');
  ...
  const performanceSmoke = read('scripts/performance-smoke.mjs');
  assert.match(performanceSmoke, /performanceSmoke\.v1/);
  assert.match(performanceSmoke, /benchmark-runtime\.mjs/);
  assert.match(performanceSmoke, /simulate-tool-scale\.mjs/);
  assert.match(performanceSmoke, /simulate-mixed-upstreams\.mjs/);
  assert.match(performanceSmoke, /simulate-upstream-failsafe\.mjs/);
  assert.match(performanceSmoke, /max-http-p95-ms/);
  assert.match(perfDoc, /benchmark:runtime/);
  assert.match(readme, /benchmark:runtime/);
  assert.match(readme, /verify:performance/);
  assert.match(perfVerification, /verify:performance/);
  assert.match(perfVerification, /host-specific runtime proof/i);
});
