...
test('connect command gives users one client-first wiring report', () => {
  ...
  assert.match(connect, /mcpace server presets/);
  assert.match(connect, /mcpace server starter/);
...
test('useful MCP installs are data-driven presets instead of more Rust package hardcode', () => {
  const presetCatalog = JSON.parse(readFileSync('presets/mcp-servers.json', 'utf8'));
  const mcpPresets = readFileSync('src/mcp_presets.rs', 'utf8');
  const serverPresets = readFileSync('src/server/presets.rs', 'utf8');
  const serverArgsNow = readFileSync('src/server/args.rs', 'utf8');
  const serverRenderNow = readFileSync('src/server/render.rs', 'utf8');
  const serverPresetRender = readFileSync('src/server/preset_render.rs', 'utf8');
  ...
  assert.equal(presetCatalog.$schema, 'mcpace.mcpPresetCatalog.v1');
  assert.ok(presetCatalog.presets.some((preset) => preset.id === 'filesystem'));
  assert.ok(presetCatalog.presets.some((preset) => preset.id === 'playwright'));
  assert.ok(presetCatalog.presets.some((preset) => preset.id === 'context7'));
  assert.ok(presetCatalog.presets.some((preset) => preset.id === 'git'));
  assert.deepEqual(presetCatalog.starter.presets, ['filesystem']);
  assert.match(JSON.stringify(presetCatalog), /@modelcontextprotocol\/server-filesystem/);
  assert.match(JSON.stringify(presetCatalog), /@playwright\/mcp@latest/);
  assert.match(JSON.stringify(presetCatalog), /@upstash\/context7-mcp/);
  assert.match(JSON.stringify(presetCatalog), /mcp-server-git/);

  assert.match(lib, /mod mcp_presets/);
  assert.match(serverRoot, /mod presets/);
  assert.match(serverRoot, /presets::install/);
  assert.match(serverRoot, /presets::starter/);
  assert.match(serverArgsNow, /server presets/);
  assert.match(serverArgsNow, /server install <preset>/);
  assert.match(serverArgsNow, /server starter/);
  assert.match(serverArgsNow, /--path <path>/);
  assert.match(serverPresets, /install_preset/);
  assert.match(serverPresets, /install_starter/);
  assert.match(serverPresetRender, /render_preset_catalog/);
  assert.match(serverPresetRender, /render_preset_install_result/);
  assert.match(serverPresetRender, /render_starter_result/);

  assert.match(mcpPresets, /presets\/mcp-servers\.json/);
  assert.match(mcpPresets, /MCPACE_MCP_PRESETS/);
  assert.match(mcpPresets, /mcpPresets", "includePaths/);
  assert.match(mcpPresets, /repository-flag/);
  assert.match(mcpPresets, /path_mode\.as_str\(\)/);
  assert.match(mcpPresets, /write_mcp_server_entry/);
  assert.doesNotMatch(mcpPresets, /@modelcontextprotocol\/server-filesystem/);
  assert.doesNotMatch(mcpPresets, /@playwright\/mcp@latest/);
  assert.doesNotMatch(mcpPresets, /@upstash\/context7-mcp/);
  assert.doesNotMatch(mcpPresets, /mcp-server-git/);
  assert.ok(releaseManifestNow.includePaths.includes('presets'));
  assert.deepEqual(configNow.mcpPresets.includePaths, ['presets/mcp-servers.json']);
  assert.ok(schemaNow.properties.mcpPresets);
});
