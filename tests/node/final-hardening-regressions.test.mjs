...
import {
	...
	resolveBinary,
} from "../../packages/npm/cli/lib/resolve-binary.js";
import {
	binaryNameForTarget,
	detectTarget,
...
test("ZIP writer rejects absolute root names and documents classic ZIP size limits", () => {
	...
	try {
		...
		assert.throws(
			() =>
				createZipFromDirectory(tmp, path.join(tmp, "out.zip"), {
					rootName: "/absolute-root",
				}),
			/ZIP root name must be relative/,
		);
		assert.match(
			read("scripts/lib/zip-writer.mjs"),
			/ZIP64 is not implemented/,
		);
		assert.match(
			read("scripts/lib/zip-writer.mjs"),
			/duplicate ZIP entry path after normalization/,
		);
	} finally {
...
test("release builder keeps portable archive path and copied-path accounting guards", () => {
	...
	assert.doesNotMatch(
		builder,
		/copied\.push\(\.\.\.result\.copied\);\s*copied\.push\(\.\.\.result\.copied\);/,
	);
...
test("resolveBinary rejects vendored binaries that escape through symlinked parent directories", (t) => {
	...
	const tmp = fs.mkdtempSync(
		path.join(os.tmpdir(), "mcpace-vendor-parent-symlink-"),
	);
...
test("Rust runtime writes use create_new temp files, fsync, private runtime dirs, and serve start locking", () => {
	...
	assert.match(serve, /file\.lock\(\)/);
	assert.match(
		serve,
		/create_new\(true\)\s*\.write\(true\)\s*\.open\(&lock_path\)/,
	);
});
