...
test("client plan counters and hub-owned stdio only count routable active routes", () => {
	...
	assert.match(
		buildPlan,
		/if route_is_routable \{\s*if matches!\(\s*plan\.execution\.mode,\s*ExecutionMode::Shared \| ExecutionMode::Pool\s*\)\s*&& plan\.parallelism_limit > 1/s,
	);
	assert.match(
		buildPlan,
	...
	assert.match(
		buildPlan,
		/server_record_is_routable\(record\)\s*&& \(record\.scope_class == "project-local"/s,
	);
	assert.match(
		buildPlan,
	...
	const planRoutable = sliceFn(
		plan,
		"server_plan_is_routable",
...
test("admission state exposes policy/platform-disabled routes before lease acquisition", () => {
	const plan = read("src", "client", "plan.rs");
	const admission = sliceFn(
		plan,
		"admission_state",
		"server_record_is_routable",
	);
	...
	assert.match(
		admission,
		/record\.source_enabled && record\.effective_enabled/,
	);
...
test("lease acquisition blocks disabled, legacy, zero-capacity, and malformed route states", () => {
	...
	const cleanName = sliceFn(
		leases,
		"clean_required_server_name",
		"non_empty_token",
	);
	assert.match(
		cleanName,
		/server_name\.chars\(\)\.any\(\|ch\| ch\.is_control\(\)\)/,
	);
});

test("destructive tool policy overrides host-readonly/browser observation inference", () => {
	const loader = read("src", "server", "loader.rs");
	const runtime = sliceFn(
		loader,
		"infer_runtime_classification",
		"policy_mentions_destructive",
	);
	...
	assert.ok(
		runtimeReadonlyIndex > -1,
		"missing browser observation read-only branch",
	);
	assert.ok(
		runtimeMutatingIndex > runtimeReadonlyIndex,
		"missing conservative host-mutating fallback",
	);
	assert.match(
		runtime,
		/\(signals\.contains\("browser-observation"\) \|\| state_binding == "host-readonly"\)\s*&& !destructive_tools/s,
	);

	const parallel = sliceFn(
		loader,
	...
	const destructiveIndex = parallel.indexOf(
		"P0_mutating_requires_serialization",
	);
	assert.ok(hostReadonlyIndex > -1, "missing host-readonly parallel candidate");
	assert.ok(
		destructiveIndex > hostReadonlyIndex,
		"host-readonly destructive fallback should reach the mutating guard",
	);
});
...
test("declared policy enum tokens are canonicalized before routing decisions", () => {
	...
	for (const key of [
		...
		assert.match(
			normalize,
			new RegExp(`policy_token\\(\\s*policy,\\s*"${key}"`),
			`${key} should use canonical token parsing`,
		);
	}

	assert.match(
		normalize,
		/let conflict_domain = policy_string\(policy, "conflictDomain"/,
	);
