...
test("security policy check is wired into CI and remains hard-green", () => {
	...
	assert.match(
		packageJson.scripts["check:security-policy"],
		/security-policy-check\.mjs/,
	);
	assert.equal(packageJson.scripts["check:ci"], "node scripts/check-ci.mjs");
	const ci = read("scripts/check-ci.mjs");
	assert.match(ci, /["']check:security-policy["']/);
	assert.match(ci, /["']check:mcp-transport["']/);
	assert.match(ci, /["']check:rust-boundaries["']/);
	assert.match(ci, /["']check:release-ready["']/);
	assert.match(ci, /["']check:supply-chain-evidence["']/);
	assert.doesNotMatch(ci, /["']proof:rust-live["']/);
	assert.match(ci, /["']check:endgame["']/);
	...
	const report = parseJson(output, "security policy check");
...
test("local HTTP and upstream guards cover SSRF/CSRF-adjacent local-boundary classes", () => {
	...
	assert.match(dashboard, /cannot use a UNC or device-network path/);
	assert.match(dashboard, /cannot contain parent traversal/);
	assert.match(
		upstream,
