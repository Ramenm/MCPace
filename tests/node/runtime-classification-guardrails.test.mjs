...
test('runtime signal inference avoids substring false positives for random MCP servers', () => {
  ...
  assert.doesNotMatch(loader, /haystack\.contains\("git"\) \|\| haystack\.contains\("repository"\)/);
  assert.doesNotMatch(loader, /mutation_terms\.iter\(\)\.any\(\|term\| haystack\.contains\(term\)\)/);
});

test('single-writer and project-isolated servers do not advertise worker-pool parallelism', () => {
  const loader = read('src', 'server', 'loader.rs');
  const inferMaxWorkers = loader.slice(loader.indexOf('fn infer_max_workers'));

  assert.match(inferMaxWorkers, /concurrency_policy == "single-writer"/);
  assert.match(inferMaxWorkers, /concurrency_policy == "isolated-per-project"/);
  assert.match(inferMaxWorkers, /return 1;/);
});

test('stateless detection is not confused with low worker limits', () => {
  const discovery = read('src', 'adapter', 'discovery.rs');
  const serverIsStateful = discovery.slice(discovery.indexOf('fn server_is_stateful'), discovery.indexOf('fn server_requires_serialization'));
...
test('runtime classifier separates read-only browser observation from browser automation', () => {
  ...
  assert.match(loader, /signals\.insert\("browser-observation"\.to_string\(\)\)/);
  ...
  const scenario = read('eval', 'fixtures', 'runtime', 'random-npm-browser-tabs-readonly.json');
...
test('random metadata guardrails avoid broad browser and SDK false positives', () => {
  ...
  assert.equal(loader.includes('"browser",\n            "browsers",\n            "screenshot"'), false);
