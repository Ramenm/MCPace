...
test("local proof script exposes a safe current-host command plan", () => {
	const result = spawnSync(
		process.execPath,
		["scripts/local-proof.mjs", "--plan-only", "--json", "--full"],
		{
			cwd: repoRoot,
			encoding: "utf8",
		},
	...
	assert.ok(
		report.results.some(
			(item) => item.id === "node-contracts" && /run check/.test(item.command),
		),
	);
...
test("platform testing instructions and local proof ship in the source bundle", () => {
	...
	assert.match(
		packageJson.scripts["proof:local:plan"],
		/local-proof\.mjs --plan-only --json/,
	);
	...
	assert.match(docs, /host-specific paths and tool locations/);
	const gitignore = read(".gitignore");
	assert.match(gitignore, /reports\/local-proof-\*\.json/);
	assert.match(gitignore, /reports\/local-proof-\*\.md/);
