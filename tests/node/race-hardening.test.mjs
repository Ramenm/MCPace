...
import {
	...
	withFileLockSync,
} from "../../scripts/lib/atomic-fs.mjs";
...
test("release builder avoids access-before-use, non-atomic copy and visible archive unlink patterns", () => {
	...
	assert.match(source, /writeFileAtomicSync\(\s*manifestPath/);
...
test("zip writer reads archived files from stable file descriptors and writes atomically", () => {
	...
	assert.match(
		source,
		/readRegularFileStableSync\(absolutePath, \{ maxBytes: maxFileBytes \}\)/,
	);
	assert.match(source, /writeFileAtomicSync\(\s*archivePath/);
...
test("stable regular-file reader rejects symlink inputs instead of following them", (t) => {
	...
	try {
		...
		assert.throws(
			() => readRegularFileStableSync(link),
			/ELOOP|symbolic|non-regular|not a regular/,
		);
	} finally {
...
test("file lock recovers dead same-host owner pids but fails closed for foreign locks", () => {
	const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "mcpace-lock-"));
	const lock = path.join(tmp, "release.lock");
	try {
		fs.writeFileSync(
			lock,
			`${JSON.stringify({ pid: 999999, host: os.hostname(), createdAtMs: Date.now() })}\n`,
		);
		assert.equal(
			withFileLockSync(lock, () => "recovered"),
			"recovered",
		);
		assert.equal(
			fs.existsSync(lock),
			false,
			"recovered lock must be cleaned after callback",
		);

		fs.writeFileSync(
			lock,
			`${JSON.stringify({ pid: 999999, host: "other-host", createdAtMs: 1 })}\n`,
		);
		assert.throws(
			() => withFileLockSync(lock, () => "unreachable"),
			/lock is already held|appears stale/,
		);
		assert.equal(
			fs.existsSync(lock),
			true,
			"foreign or ambiguous locks must not be removed automatically",
		);
	} finally {
		fs.rmSync(tmp, { recursive: true, force: true });
	}
...
test("clean child environment drops common compiler, loader, Node and npm poisoning knobs", () => {
	...
	for (const key of [
		...
		assert.equal(
			Object.hasOwn(cleaned, key),
			false,
			`${key} must not be forwarded by default`,
		);
	}
});
...
test("publish workflow forces manual dispatch through dry-run planning", () => {
	const workflow = read(".github/workflows/publish-npm.yml");
	assert.match(
		workflow,
		/MCPACE_PUBLISH_DRY_RUN:\s*>-\s*\n\s*\$\{\{\s*github\.event_name == 'workflow_dispatch'\s*\}\}/,
	);
	assert.doesNotMatch(workflow, /^\s+dry_run:\s*$/m);
	assert.match(
		workflow,
		/MCPACE_VERSION_OVERRIDE:\s*>-\s*\n\s*\$\{\{[\s\S]*?inputs\.version_override[\s\S]*?\}\}/,
	);
