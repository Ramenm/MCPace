...
fn build_next_steps(
    ...
    if upstream.configured_count == 0 {
        steps.push("Start with curated useful MCP presets: mcpace server presets".to_string());
        steps.push(
            "Preview the conservative local starter: mcpace server starter --path . --dry-run"
                .to_string(),
        );
