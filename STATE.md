...
- source/archive/npm checks run in this container; current-host Rust build/test proof is still blocked here because `cargo`/`rustc` are not available in this environment; live Docker/real-client runtime proof also does **not** run here
...
- **beta-only activation still missing:** real-host proof that upstream tool calls run through durable cross-process MCPace session/process ownership; request-time wrappers now have heartbeat renewal, settings-only conservative leases, active lease-session bookkeeping, bounded in-process session pooling with resource-derived defaults, metadata/header-derived session affinity, bounded local HTTP request handling with a fixed worker pool, live runtime telemetry for health/overview, and lost-lease/stale-id guards in source tests
